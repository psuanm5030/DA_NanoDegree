...
var scrollVis = function() {
  ...
  var chart = function(selection) {
    selection.each(function(rawData) {
      ...
      svg = d3.select(this).selectAll("svg").data([tData]);
      ...
      var tData = getWords(rawData);
      // filter to just include filler words
      var fillerWords = getFillerWords(tData);

      // *** Get the PIE data 
      var pieData = getPieAgg(tData);
      ...
      setupVis(tData, fillerCounts, histData);
   ...
   *
   * @param tData - data object for each passenger.
   * @param fillerCounts - nested data that includes
   *  element for each filler word type.
   * @param histData - binned histogram data
   */
  setupVis = function(tData, fillerCounts, histData, pieData) {
    // axis
    g.append("g")
      .attr("class", "x axis")
    ...
    // square grid
    var squares = g.selectAll(".square").data(tData);
    squares.enter()
      .append("rect")
  ...
  /**
   * showPie - pie chart of survivability
   *
   * hides: passenger count title (filler count title)
  ...
  /**
   * getPieAgg - returns the data as an array of two objects, containing the counts of survivors
   *
   * @param data - base dataset
   */
  function getPieAgg(data) {
    return  d3.nest()
...
d3.tsv("data/titanic_full.csv", display);
