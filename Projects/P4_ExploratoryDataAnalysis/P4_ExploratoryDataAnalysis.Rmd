...
rw <- read.csv('/Users/Miller/GitHub/DAnanodegree/Projects/P4_ExploratoryDataAnalysis/reds.csv', sep = ',', colClasses = c('character','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric','numeric'))
```
...
# Zoom in on the left portion of the x-axis - see how many wines are less than 1 gram
ggplot(aes(residual.sugar), data = rw) + 
  geom_histogram(binwidth = .1) + 
  scale_x_continuous(breaks = seq(0,16,1)) + 
...
mean(rw[rw$fixed.acidity>12,'pH']) # Mean of 3.08 is versus full dataset of 3.3 - roughly 1.5 standard deviations from the mean.
```
...
```{r Correlations}
# Correlation Table
rw.num <- rw[, seq(2,13,1)] # Take all original variables, except the ID field (column 1)
...
# Creating a new variable that assigns one of two text values (Above or Below Average) based upon the quality value.
summary(rw$quality) # Mean of 5.636 - use 5 as the 'Average' value
# Using only two categories for easier interpretation (i.e., no "Average" category).
rw$qTerm <- ifelse(rw$quality <= 5, 'Below Average', 'Above Average')
table(rw$qTerm) # Check the counts

# Show a box plot
boxplot(rw$alcohol ~ rw$qTerm) # See that the difference between "Above Average" and the others visually significant.
```
...
# Faceted distribution - This is clear indication of the skewedness of below average wines (less alcohol).
ggplot(aes(alcohol), data = rw) + geom_histogram(binwidth = .1) + facet_wrap(~ qTerm)
...
ggplot(rw,aes(alcohol, volatile.acidity)) + 
  geom_point(aes(color = quality), alpha = 3/4) + 
  geom_smooth(method = 'lm') + 
  scale_color_gradient2(low = muted("red"), mid = "white", high = muted("blue"), midpoint = 5)
...
# Segregate into equal intervals (i.e., low, medium and high amounts of volatile acidity)
...
ggplot(aes(log(chlorides), log(alcohol), color = as.factor(quality)), data = rw) + 
...
# I remember that the variable is very right-skewed.  However a log transformation of is much more normal.
...
summary(rw$total.sulfur.dioxide) # Mean is 46.47 - we know effects are possible beyond 50
# Divide the values into two buckets
rw$soCat <- cut(rw$total.sulfur.dioxide, breaks = c(0,50,300), labels = c('No Effect','Potential Effect'))
...
# Remember, we showed that sulphates were were right-skewed in the histogram above.  Lets take a look at its linkage to quality.
...
**Final Plot #1**
```{r First Final Plot}
# The Frequency polygon plot clearly points out the difference between higher quality and lower quality wines.
ggplot(aes(alcohol, ..count../sum(..count..)), data = rw) + 
  geom_freqpoly(aes(color = qTerm), binwidth = .1) + 
...
ggplot(rw,aes(log(sulphates),log(alcohol))) + 
  geom_point(aes(color = as.factor(round(quality)))) + 
  scale_color_brewer(palette = 'Greens', name = 'Quality Rating',guide = guide_legend(reverse=TRUE)) + 
  geom_smooth(method = 'lm') + 
...
ggplot(aes(quality, fill = vaCat), data = rw) + 
  geom_bar() + 
  scale_fill_brewer(type = 'seq', palette = 3, name = 'Volatile Acid Content',guide = guide_legend(reverse=TRUE),labels=c("Low", "Medium", "High")) + 
  xlab('Quality Rating') + 
  ylab('Count of Observations (Wines)') + 
  ggtitle('No. of Wines by Quality Rating (by Interval of Volatile Acid)') + 
  theme(panel.background = element_rect(fill = 'white',colour = 'red'),panel.grid.major = element_line(colour="grey"),panel.grid.minor = element_line(colour="grey"))
```
